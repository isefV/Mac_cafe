...
const BackgroundItemComponent = () => {
    ...
    // <img src="./assets/img/sphere.png" alt="3d"/>,
    // <img src="./assets/img/coffee-bean.png" alt="3d"/>,
    // <img src="./assets/img/cup.png" alt="3d"/>
    <img src="./assets/svg/smile.svg" alt="3d"/>

]
    // const [ballsInfo,setBallsInfo] = useState([]);
    
    const ballsInfo = useMemo(()=>{
        const ballsCount = Math.floor(Math.random() * 3) + 3;
        // const ballsCount = 3;
    ...
    return ( <div className="bg-Cntr">
        {ballsInfo.length !== 0 && ballsInfo.map((item,index)=>{ 
            const randsvg = Math.floor(Math.random() * svgs.length);
            return <div className="bg-item bg-animations center" style={item} key={"bg-"+index}>{svgs[randsvg]}</div> })}

            {/* {svgs.length!==0 && svgs.map((item,index)=>{
                return <div className="bg-item bg-balls center" key={"bg-"+index}>{item}</div>
            })} */}
